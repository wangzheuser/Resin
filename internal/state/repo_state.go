...
func (r *StateRepo) GetSystemConfig() (*config.RuntimeConfig, int, error) {
	...
	return config.NormalizeRuntimeConfig(cfg), version, nil
}
...
func (r *StateRepo) UpsertSubscription(s model.Subscription) error {
	...
	_, err := r.db.Exec(`
			INSERT INTO subscriptions (id, name, source_type, url, content, relay_platform_id, update_interval_ns, enabled,
			...
			ON CONFLICT(id) DO UPDATE SET
				...
				relay_platform_id  = excluded.relay_platform_id,
				...
				updated_at_ns      = excluded.updated_at_ns
		`, s.ID, s.Name, s.SourceType, s.URL, s.Content, s.RelayPlatformID, s.UpdateIntervalNs, s.Enabled,
...
func (r *StateRepo) ListSubscriptions() ([]model.Subscription, error) {
	rows, err := r.db.Query(`SELECT id, name, source_type, url, content, relay_platform_id, update_interval_ns, enabled,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.Name, &s.SourceType, &s.URL, &s.Content, &s.RelayPlatformID, &s.UpdateIntervalNs, &s.Enabled,
