...
### 为订阅节点配置单跳前置 Platform

当订阅中的目标节点需要先经过一批已可达节点再建立连接时，可在订阅创建或编辑页面选择 **「前置 Platform」**：

1. 先创建一个 Platform，并用标签、地区等规则纳入适合作为前置出口的节点。
2. 在订阅的「前置 Platform」选择框中引用该 Platform；留空表示直连。
3. Resin 在每次连接目标节点服务器时，从该 Platform 当前的健康直连节点中轮询选择，单次连接最多尝试 3 个候选。

该配置仅作用于订阅解析出的**节点出站连接**，远程订阅 URL 的下载路径保持原有行为。链路固定为单跳：`Resin → 前置 Platform 节点 → 订阅节点`。候选集为空或尝试耗尽时，本次连接以错误结束，直连路径不参与回退。带 `detour` 的节点和本身已引用前置 Platform 的节点会从候选集中排除。被订阅引用的 Platform 需要先解除引用，再执行删除。
